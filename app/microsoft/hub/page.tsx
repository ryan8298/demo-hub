          ...
          {demos.map((demo) => (
            <div key={demo.id} className="bg-slate-800 rounded-lg overflow-hidden hover:shadow-lg transition cursor-pointer">
              {demo.preview_image_url && (
                <img src={demo.preview_image_url} alt={demo.title} className="w-full h-48 object-cover" />
              )}
              <div className="p-6">
                <h3 className="text-xl font-bold text-white mb-2">{demo.title}</h3>
                <p className="text-slate-400 text-sm mb-4">{demo.description}</p>
                <button onClick={() => setExpandedId(expandedId === demo.id ? null : demo.id)} className="text-blue-400 hover:text-blue-300 font-semibold text-sm">
                ...
                {expandedId === demo.id && (
                  <div className="mt-4 pt-4 border-t border-slate-700 space-y-4">
                    ...
                    <a href={demo.demo_url} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm font-semibold transition">
                      Open Demo →
                    </a>
                  </div>
                )}
