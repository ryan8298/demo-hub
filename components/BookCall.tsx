...
export function BookCallModal({
  ...
  return (
    <Modal open={open} onClose={onClose} labelledBy="bookcall-title">
      <button
        type="button"
        onClick={onClose}
      ...
      {hasUrl ? (
        <div className="rounded-xl overflow-hidden border border-milk/10 bg-white">
          {/* Outlook Bookings embed. Fixed height keeps the calendar usable
              inside the modal's max-h scroll area. */}
          <iframe
            src={BOOKINGS_URL}
